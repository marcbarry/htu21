...
python enclosure/rpi3bplus/v1/build.py --openscad PATH/TO/openscad.com
...
PARTS = {"base": "pi3bplus_base", "lid": "pi3bplus_lid",
         "sensor": "htu21_sensor_arm", "test": "sensor_mount_test"}
...
def main():
    ...
    if args.checks:
        for part in ["check_case_arm", "check_base_lid", "check_pcb", "check_pi_pcb", "check_withdrawal"]:
    ...
    if args.renders:
        for part in ["base", "lid", "sensor", "test", "assembly", "exploded", "interface"]:
            output = ROOT / "renders" / (part+".png")
            output.parent.mkdir(exist_ok=True)
            camera = ["--camera=0,0,0,60,0,210,200"] if part in ["assembly", "exploded", "interface"] else []
            result = subprocess.run([args.openscad, "-D", f'part="{part}"', "-o", str(output), "--render",
